...
-  조회
  ...
  -  무한스크롤
      ...
      - 페이지가 담는 정보의 양이 많아짐에 따라 사용자가 이탈하지 않고 해당 페이지에 머물면서 정보를 쉽게 볼수있도록 구현 
  -  top button footer
...
-  커뮤니티 게시글, 채용사이트 글, 댓글 검색 가능
  - 사용자가 필요한 정보나 채용글을 검색을 통해서 볼수 있도록 구현 
  - 채용, 댓글, 게시글을 나눔으로서 해당 검색 정보가 어디에 존재 하는지 알수 있도록 페이지 구분(비동기)
...
자극 받아서 부족한 실력이지만 팀장으로서 최선을 다하려고 노력했던것 같습니다. 익숙하지 않은 프론트엔드 작업이라 서툴기도 하고 이전에 배웠던 내용들에 대해서 기억이 나지 않아 작은 부분을 구현하는데도 많은 시간이 걸리고 오래 붙잡고 있다보니 능률적으로 참여하지 못한거 같아 아쉽지만 그래도 배워가는 부분은 있어 다행인것 같습니다.
